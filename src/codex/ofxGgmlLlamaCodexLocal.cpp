...
std::filesystem::path codexAgentRoleDirectory(const std::string & configPath) {
	return codexConfigParent(configPath) / "agents";
}
...
std::string localAgentRoleToml(
	...
	const auto providerLabel = config.providerName.empty() ? "local provider" : config.providerName;
	output << "# Generated by ofxGgmlLlamaCodexLocal.\n";
	output << "name = \"" << escapeTomlString(role) << "\"\n";
	if (role == "explorer") {
		output << "description = \"Read-only local codebase explorer using " <<
			escapeTomlString(providerLabel) << ".\"\n";
	} else {
		output << "description = \"Execution-focused local worker using " <<
			escapeTomlString(providerLabel) << ".\"\n";
	}
...
bool writeAgentRoleFiles(
	...
	const auto explorerPath = directory / "explorer.toml";
	const auto workerPath = directory / "worker.toml";
...
bool ofxGgmlLlamaCodexLocal::startLlamaServer(
	...
	args << " --ctx-size " << std::max(0, settings.contextSize);
	...
	if (executableSupportsArgument(settings.serverExe, "--kv-unified")) {
		args << " --kv-unified";
	}
	...
	if (!settings.kvCacheKeyType.empty() &&
		executableSupportsArgument(settings.serverExe, "-ctk")) {
		args << " -ctk " << settings.kvCacheKeyType;
	}
	if (!settings.kvCacheValueType.empty() &&
		executableSupportsArgument(settings.serverExe, "-ctv")) {
		args << " -ctv " << settings.kvCacheValueType;
	}
