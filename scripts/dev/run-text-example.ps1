...
if ($Build) {
	& (Join-Path $scriptRoot "build-example.ps1") -Example "text" -Configuration $Configuration -Platform $Platform
...
if ((Test-Path -LiteralPath $exampleExe -PathType Leaf)) {
	...
	throw "Text example executable was not found: $exampleExe. Run scripts\run-example.bat text -Build first."
}
...
Write-OfxGgmlStep "Starting ofxGgmlTextExample"
& $exampleExe
exit $LASTEXITCODE
